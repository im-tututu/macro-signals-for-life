...
from src.core.config import TABLE_RAW_JISILU_GOLD
...
GOLD_SPEC = TableSpec(
    ...
    numeric_fields=GOLD_NUMERIC_FIELDS,
...
@dataclass
class GoldEtfStore(BaseSqliteStore):
    """集思录黄金 ETF 原始表 store。"""

    spec: TableSpec = GOLD_SPEC
    ...
    def build_row_from_history_cell(
        ...
        *,
        ...
        return {
            ...
            "index_nm": str(cell.get("index_nm") or cell.get("index_name") or ""),
    ...
    @classmethod
    def build_rows_from_fetch_result(cls, fetch_result: FetchResult[JisiluGoldSnapshot]) -> list[dict[str, Any]]:
        ...
        if not snapshot_date:
            raise ValueError("Gold snapshot missing snapshot_date")
        if not fetched_at:
            raise ValueError("Gold snapshot missing fetched_at")
        if not rows:
            raise ValueError("Gold snapshot rows are empty")
        return [
