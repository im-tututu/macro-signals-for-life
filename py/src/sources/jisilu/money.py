...
JISILU_MONEY_LIST_URL = "https://www.jisilu.cn/data/etf/money_list/"


class JisiluMoneyEtfSource(BaseSource):
    """集思录货币 ETF / 货币类列表来源。"""
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.http.session.headers["Referer"] = JISILU_MONEY_LIST_URL
    ...
    def _notify_cookie_expired(self) -> None:
        ...
        self._notifier.notify(
            "Jisilu Cookie 可能失效",
            "货币 ETF 抓取检测到游客 20 条限制，已尝试刷新 cookie。",
            level="WARNING",
        )
        self._cookie_expired_notified = True

    def fetch_money_page(
        self,
        *,
        page: int = 1,
        ...
        response = self.http.request("GET", JISILU_MONEY_LIST_URL, params=params)
    ...
    def fetch_money_all(
        ...
        *,
        ...
        while True:
            try:
                return self._fetch_money_all_once(
    ...
    def _fetch_money_all_once(
        ...
        *,
        ...
        fetched_at = now_text()
        row_snapshots: List[JisiluMoneyRowSnapshot] = []
        for row in self.require_rows(all_rows):
            cell = row.get("cell", row) if isinstance(row, dict) else {}
            fund_id = self.require_text(str(cell.get("fund_id") or row.get("id") or ""), field_name="fund_id")
            row_snapshots.append(JisiluMoneyRowSnapshot(fund_id=fund_id, cell=dict(cell), raw_row=dict(row)))

        return JisiluMoneySnapshot(
            snapshot_date=target_snapshot_date,
            fetched_at=fetched_at,
            source_url=JISILU_MONEY_LIST_URL,
    ...
    def fetch_money_result(
        ...
        *,
        ...
        min_unit_total_yi: float | int | str = "",
        min_volume_wan: float | int | str = "",
    ) -> FetchResult[JisiluMoneySnapshot]:
        snapshot = self.fetch_money_all(
            snapshot_date=snapshot_date,
            rows_per_page=rows_per_page,
            max_pages=max_pages,
            min_unit_total_yi=min_unit_total_yi,
