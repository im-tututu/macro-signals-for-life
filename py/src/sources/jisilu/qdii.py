...
JISILU_QDII_DETAIL_HISTS_URL = "https://www.jisilu.cn/data/qdii/detail_hists/"
JISILU_QDII_DETAIL_URL = "https://www.jisilu.cn/data/qdii/detail/{fund_id}"


class JisiluQdiiEtfSource(BaseSource):
    """集思录 QDII 来源。
    ...
    def fetch_qdii_detail_history_page(
        self,
        *,
        fund_id: str,
        page: int = 1,
        ...
        headers = {
            "Referer": JISILU_QDII_DETAIL_URL.format(fund_id=fund_id),
            "X-Requested-With": "XMLHttpRequest",
        }
        response = self.http.request(
            "POST",
            JISILU_QDII_DETAIL_HISTS_URL,
            headers=headers,
            data={
                "is_search": 1,
                "fund_id": fund_id,
        ...
        if self._detect_tourist_limit(raw_text, payload) or self._looks_like_tourist_rows(rows, records):
            self._notify_cookie_expired(market="europe_america")
